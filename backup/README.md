# Gondola v2.2 (Necromancer Edition)

## Overview
Gondola v2.2 introduces the **Necromancer Architecture**: a stateless, resilient, and high-precision system designed to survive process deaths, interruptions, and "implementation amnesia."
...
### 6. Semantic Search (Conceptual Navigation)
*   **Mechanism:** Local vector embeddings using `sentence-transformers` and `all-MiniLM-L6-v2`.
*   **Tool:** `semantic_search`
*   **Benefit:** Allows the agent to find code based on meaning (e.g., "where is the theme handled?") even if exact keywords aren't known.

---

## The Next Move: Phase 7 - The Oracle Architecture (Behavioral Understanding)

The goal is to move from "searching" to "understanding" by mapping the project's behavioral nervous system.

### 1. Neural Dependency Mapping
*   **Mechanism:** Trace function call chains and data flow across files using enhanced Tree-sitter analysis.
*   **Impact:** The agent knows that changing `A.py` will break `B.py` before it even runs the code.

### 2. Automated Verification (The Sentry)
*   **Mechanism:** Automatically identifies and executes relevant test cases based on the dependency graph after every edit.
*   **Impact:** Immediate feedback loops that guarantee implementation integrity.

### 3. Cognitive Flashcards
*   **Mechanism:** Automated generation of "Memory Fragments" after complex bug fixes to prevent regression and implementation amnesia.
...
*   **Tools Available:**
    ...
    - Semantic Search: conceptual navigation and hybrid scoring
    - Memory: persistent notes, preferences, and "save_knowledge"
...
*Gondola v2.2: Built for resurrection, not persistence.*

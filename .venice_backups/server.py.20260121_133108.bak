...
from venice.prompts.system import SYSTEM_PROMPT
from venice.api import get_api_key, execute_tool
...
def initialize():
    ...
    try:
        ...
        api_key = get_api_key()
        if not api_key:
            print("WARNING: VENICE_API_KEY not found. Chat will fail.")
        else:
...
def edit_image():
    try:
        ...
        if image_b64:
            ...
            # Minimal payload for Edit endpoint (based on error message)
            payload = {
                "prompt": prompt,
            ...
            try:
                ...
                while not stop_signal:
                    ...
                    try:
                        ...
                        with httpx.Client(timeout=httpx.Timeout(stream_timeout + 60.0, read=stream_timeout)) as http_client:
                            with http_client.stream(
                                ...
                                if "application/json" in content_type:
                                    ...
                                    print(f"DEBUG API JSON: {error_json}")
                                    try:
                    ...
                    if agent_turns >= MAX_AGENT_TURNS:
                        messages.append({"role": "user", "content": "SYSTEM: Max turns reached."})
                        break
            except Exception as e:
                traceback.print_exc()
                event_queue.put({"type": "error", "data": str(e)})
...
if __name__ == '__main__':
    print("Starting Gondola Server (Modular) on port 5009...")

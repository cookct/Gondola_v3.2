...
from cortex.config import SCRIPT_DIR
from cortex.core import UI, MODELS
...
def _get_key_from_config(key_name, env_var_name):
    ...
    2. Config files (app_config.json, ~/.cortex_config.json, ~/.config/cortex/config.json)
    
    Args:
        key_name: The config key name to look for in JSON files (e.g., "cortex_api_key")
        env_var_name: The environment variable name to check (e.g., "CORTEX_API_KEY")
    ...
    config_locations = [
        ...
        os.path.expanduser("~/.cortex_config.json"),
        os.path.expanduser("~/.config/cortex/config.json"),
        "app_config.json",
    ]
...
def get_api_key():
    """Retrieve the Cortex API key from environment or config files."""
    return _get_key_from_config("cortex_api_key", "CORTEX_API_KEY")

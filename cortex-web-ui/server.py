...
# Add parent dir to path to import cortex package
...
from cortex.core import UI, MODELS, get_model_config
from cortex.config import PERSONALITY_FILE
from cortex.workspace import Workspace
from cortex.memory import Memory
from cortex.tools import CombinedTools
from cortex.prompts.system import SYSTEM_PROMPT, build_system_prompt, get_checkpoint_message
from cortex.api import get_api_key, get_together_api_key, get_zai_api_key, execute_tool, parse_tool_call
...
from cortex.context_manager import ContextManager, ModelConfig
from cortex.agent_state import AgentState
from cortex.persistence import PersistenceManager
import httpx
...
# ============================================================================
# NOTE: Both Cortex AI and Together AI now support base64-encoded images directly.
...
def initialize(clear_messages=False):
    ...
    try:
        ...
        api_key = get_api_key()
        if not api_key:
            logger.error("CORTEX_API_KEY not found - chat will fail!")
        else:
...
def list_backups():
    ...
    try:
        backup_dir = os.path.join(WORKSPACE_DIR, '.cortex_backups')
...
# Cortex AI: Uses OpenAI-compatible tool calling for most instruction-tuned models
KNOWN_FUNCTION_CALLING_MODELS = {
    ...
    # === Cortex AI Models (OpenAI-compatible tool calling) ===
...
def infer_function_calling_config(model_id: str, model_type: str, context_length: int, provider: str) -> dict:
    ...
    Both Together AI and Cortex AI use OpenAI-compatible tool calling.
    """
    # Check if it's a known function-calling model (either provider)
    is_known_fc_model = model_id in KNOWN_FUNCTION_CALLING_MODELS

    # Cortex models: Most instruction-tuned models support OpenAI tool calling
    # Be more permissive for Cortex since their API handles tool calling uniformly
    is_cortex = provider == 'cortex'
    ...
    # Cortex: More permissive - their API handles tool calling uniformly for most models
    # Together: Rely on known list + inference
    if is_cortex:
        # Cortex uses OpenAI-compatible tool calling broadly
    ...
    config = {
        ...
        "_inferred": {
            ...
            "provider": provider,
            "cortex_inference": is_cortex and not is_known_fc_model
        }
    }
...
def chat():
    ...
    if image_data:
        ...
        # Together AI and Cortex both have limits on base64 size
    ...
    def generate():
        ...
        def run_llm_and_tools():
            ...
            try:
                ...
                from cortex.core import MODELS as CORE_MODELS
                ...
                while not stop_signal and not interrupt_flag.is_set():
                    ...
                    try:
                        ...
                        from cortex.core import MODELS
                        ...
                        model_provider = model_info.get("provider", "cortex")  # Default to Cortex
                        ...
                        if not is_vision_model and any(isinstance(m.get('content'), list) for m in sanitized_messages):
                            ...
                            helper_api_key = get_api_key() # Cortex API key
                            if helper_api_key:
                                try:
                                    import httpx
                            ...
                            try:
                                with httpx.Client(timeout=httpx.Timeout(connect=60.0, read=180.0, write=60.0, pool=60.0)) as http_client:
                                    with http_client.stream(
                                        ...
                                        balance_usd = response.headers.get('x-cortex-balance-usd') or response.headers.get('X-Cortex-Balance-USD')
                                        balance_vcu = response.headers.get('x-cortex-balance-vcu') or response.headers.get('X-Cortex-Balance-VCU')
                                        ...
                                        if "application/json" in content_type:
                                            ...
                                            try:
                                                ...
                                                if "timed-out" in str(err_msg).lower() or "timeout" in str(err_msg).lower():
                                                    logger.warning(f"[{request_id}] Cortex timeout detected - this is a server-side issue")
                                                    event_queue.put({"type": "error", "data": f"Cortex API timeout - the model is overloaded. Please try again or switch to a different model."})
                                                else:
...
def get_avatar_expressions():
    ...
    from cortex.prompts.system import is_avatar_expressions_enabled
    try:
...
def set_avatar_expressions():
    ...
    from cortex.prompts.system import set_avatar_expressions_enabled
    try:
        data = request.get_json()
...
def get_edit_model_api():
    ...
    from cortex.prompts.system import get_edit_model
    try:
...
def set_edit_model_api():
    ...
    from cortex.prompts.system import set_edit_model
    try:
        data = request.get_json()
